...
namespace Sitecore.Support.XA.Feature.ErrorHandling.Pipelines.HtpRequestProcessed
{
  public class HandleServerErrorCode : HttpRequestProcessor
  {
      ...
      if (args.Context.Error != null && !Context.Site.Name.Equals("shell"))
      {
        ...
        if (args.Context.Error is HttpException)
        {
          ...
          if (error.GetHttpCode() == 404)
          {
            ...
            {
              var errorPageItem =
                LinkManager.GetItemUrl(Context.Database.GetItem(settingsItem[Sitecore.XA.Feature.ErrorHandling.Templates._ErrorHandling.Fields.Error404Page] ?? ""));

              if (Settings.RequestErrors.UseServerSideRedirect)
                HttpContext.Current.Server.TransferRequest(errorPageItem);
              else
                HttpContext.Current.Response.Redirect(errorPageItem);
    ...
    {
      return $"{ErrorPagesFolder}/{site.Name}.html";
    }

    private const string ErrorPagesFolder = "/ErrorPages";
  }
}